...
const ARTIFACTS = Object.freeze({
  ...
  RecoveryIntentBoard: ["out", "RecoveryIntentBoard.sol", "RecoveryIntentBoard.json"],
...
const BEHAVIORS = Object.freeze({
  ...
  "RecoveryIntentBoard.announce": "Emits an unverified recovery discovery hint without writing storage or gaining account authority.",
  "RecoveryIntentBoard.publishApproval": "Verifies exactly one live guardian approval, then emits the portable approval tuple for off-chain reassembly without changing recovery state.",
  "RecoveryIntentBoard.publishCancellation": "Verifies exactly one guardian objection to the currently pending recovery, then emits it for off-chain quorum assembly without cancelling anything itself.",
...
const CONTRACT_PROFILES = Object.freeze({
  ...
  RecoveryIntentBoard: { layer: "recovery-discovery", requirement: "optional", responsibility: "Optional ownerless, storage-free publication surface for recovery announcements, individual guardian approvals, and cancellation approvals. It emits discovery logs only, is never installed as a module, and cannot propose, cancel, execute, or block recovery." },
...
export function buildDeploymentEvidence({ repoRoot, addresses, codeHashes, account }) {
  ...
  const edges = [
    ...
    relationship("RecoveryIntentBoard", "RecoveryManager", "reads-recovery-state", "reads nonce, digest, or pending recovery; emits logs only"),
    relationship("RecoveryIntentBoard", "ObservedAccount", "reads-account-state", "checks installed manager, config version, and guardian root"),
    relationship("RecoveryIntentBoard", "ECDSAGuardianVerifier", "verifies-with", "optional one-guardian ECDSA publication proof"),
    relationship("RecoveryIntentBoard", "P256GuardianVerifier", "verifies-with", "optional one-guardian P-256 publication proof"),
    relationship("RecoveryIntentBoard", "ERC1271GuardianVerifier", "verifies-with", "optional one-guardian contract publication proof"),
    relationship("P256RecoveryValidatorFactory", "P256RecoveryValidator", "creates", "CREATE2 already-initialized recovery-bound child"),
    ...
    relationship("P256RecoveryValidatorFactory", "ObservedAccount", "provisions-for", "validator address binds account, nonce, key, and rotated guardian set"),
    ...
    relationship("P256RecoveryValidator", "ObservedAccount", "reserved-for", "already initialized for one exact recovery intent"),
