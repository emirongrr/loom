...
contract ImplementationLockValidatorTest is Test {
    ...
    function testInitializedImplementationCannotValidateOrBeReinitialized() external {
        ...
        userOp.sender = address(implementation);
        userOp.signature = envelope;
        assertEq(implementation.validateUserOp(userOp, bytes32(uint256(1)), 0), ValidationDataLib.SIG_VALIDATION_FAILED);
        assertEq(implementation.isValidSignature(bytes32(uint256(1)), envelope), bytes4(0xffffffff));

        vm.expectRevert(LoomAccount.InvalidDirectExecution.selector);
        implementation.executeDirect(address(validator), bytes32(0), "", type(uint48).max, "");

        vm.expectRevert(LoomAccount.OnlySelf.selector);
        implementation.scheduleCall(address(implementation), 0, "", 1 days);
