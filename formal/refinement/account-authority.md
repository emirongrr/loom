...
|---|---|---|---|---|
| `check_InitializedAccountCannotBeReinitialized` | Initialization is one-shot. | `initialized_state_rejects_reinitialization` | `initializeDelegatedAccount`, `_initialize` | The Solidity property self-calls the delegated initializer, because `initialize` is rejected earlier by the initialization-context guard and would mask the `configVersion != 0` check. It requires `InvalidInitialization` and preserves entry point, complete validator identity/nonce, guardian, freeze, and configuration state. Lean abstracts caller, exact revert data, and initialization payload. |
| `check_LiveAccountRejectsProxyInitializer` | The proxy bootstrap initializer is unreachable once an account has code. | Not modeled; Lean has no bytecode-presence predicate. | `initialize`, initialization-context guard | The Solidity property requires `InvalidInitializationContext`, so it cannot be satisfied by the one-shot guard, and preserves the complete authority snapshot. It assumes `extcodesize` is zero only inside a constructor. |
| `check_UninitializedDelegatedAccountRejectsProxyInitializer` | A third party cannot configure an uninitialized delegated account. | Not modeled; Lean has no delegation-context predicate. | `initialize`, initialization-context guard | Models an EIP-7702 delegation by etching the Loom runtime at a fresh address, so Loom storage is empty and `configVersion == 0` — the state both one-shot properties assume away. Requires `InvalidInitializationContext` and asserts no entry point, guardian, or module was installed. Etched code has the runtime's full length rather than a 23-byte delegation indicator; both are non-zero, which is what the guard reads. |
| `check_DelegatedInitializerRejectsExternalCaller` | Delegated initialization is self-only. | `external_delegated_initializer_preserves_state` | `initializeDelegatedAccount` | The Solidity property requires `InvalidInitialization` and preserves complete observable authority. Lean abstracts the caller predicate to a self-call Boolean; proxy storage, exact revert data, and initialization payload remain concrete. |
...
| `check_NoMutableUpgradeSelectorsThroughProxy` | The immutable proxy exposes no upgrade transition. | `immutable_proxy_has_no_upgrade_transition` | `LoomAccountProxy.implementation`, fallback path | The Solidity property requires empty revert data for each absent upgrade/admin selector and preserves proxy authority state. The theorem abstracts bytecode selectors and delegatecall semantics. |
| `check_InvalidDirectExecutionDoesNotConsumeNonce` | Rejected direct execution preserves its nonce. | `rejected_direct_execution_preserves_nonce` | `executeDirect`, validator nonce storage | The Solidity property requires `InvalidDirectExecution` and preserves installed and uninstalled validator nonces, authority, and target state. Lean models one abstract validator nonce; signature digest, validator identity, and nonce-map isolation remain concrete. |
| `check_BatchExecutionAtomicity` | A reverting ordinary batch leaves no partial effect. | `failed_batch_preserves_state`, `successful_batch_commits_all_effects` | `_executeAuthorized`, batch execution loop | The Solidity property impersonates the EntryPoint and requires the exact downstream target revert, preventing caller-authorization rejection from satisfying the claim. Lean abstracts external calls to a committed effect counter and does not model EVM revert data. |
| `check_FrozenAccountCannotExecute` | Freeze blocks ordinary EntryPoint execution. | `frozen_blocks_ordinary_execution` | `execute`, `_executeAuthorized` | The Solidity property requires `AccountFrozen` and preserved target, freeze, configuration, and validator state. Lean collapses EntryPoint and validator authorization into an actor. |
| `check_FrozenAccountCannotDirectExecute` | Freeze blocks ordinary direct execution. | `frozen_blocks_ordinary_execution` | `executeDirect`, `_executeAuthorized` | The Solidity property requires `AccountFrozen` and preserved direct nonce and authority state. Direct nonce/signature behavior is not modeled in Lean. |
| `check_DirectBatchExecutionAtomicity` | A reverting direct batch preserves effects and nonce. | `failed_batch_preserves_state` | `executeDirect`, batch loop, direct nonce | The Solidity property requires the exact downstream target revert. Lean abstracts nonce and external calls; concrete rollback remains covered by the Solidity property. |
| `check_GuardianlessBootstrapHasNoGuardianAuthority` | An empty guardian bootstrap grants no guardian power. | `guardianless_bootstrap_has_no_guardian_authority` | constructor guardian fields, `freeze`, `setGuardianConfig` | The Solidity property requires `OperationNotReady` and `InvalidModule`, then preserves authority and guardian nonce state. Lean abstracts guardian configuration to a Boolean; Merkle proof and exact revert semantics remain concrete. |
| `check_ExternalCannotSetGuardianConfig` | External callers cannot change guardian authority. | `external_guardian_config_preserves_state` | `setGuardianConfig`, self-call guard | The Solidity property requires `OperationNotReady` and preserves the full observable authority snapshot. Lean abstracts guardian root and threshold to the configuration-version state. |
| `check_UnsupportedExecutionModeNeverExecutes` | Unsupported ERC-7579 modes cannot call targets. | `unsupported_execution_mode_preserves_state` | `execute`, execution-mode decoder | The Solidity property impersonates the EntryPoint, requires `UnsupportedExecutionMode`, and preserves target and authority state. Lean abstracts mode support to a Boolean and does not model target calldata decoding. |
| `check_ExternalCannotRecoverConfiguration` | External callers cannot invoke account-internal recovery. | `external_recovery_preserves_authority_state` | `recoverConfiguration`, `recoverConfigurationSet` | The Solidity property requires `InvalidModule` and preserves guardian and validator identity. Lean abstracts the recovery-module caller predicate; validator replacement payload and proof routing remain concrete. |
| `check_CannotRemoveLastValidator` | Every successful transition retains a validator. | `successful_step_preserves_validator_nonzero` | `_uninstallModule`, validator-set validation | The Solidity property requires the bubbled `InvalidModule` error and rollback of the consumed schedule and complete authority snapshot. Concrete coverage is selected paths, not every future module transition. |
| `check_ConfigUpdateInvalidatesStaleSchedule` | Authority changes invalidate stale scheduled calls. | `scheduled_operation_rejects_config_change` | `configVersion`, `configHash`, scheduled calls | The Solidity property requires `OperationNotScheduled`, preserves the updated authority state, and proves no target effect. Lean models the scheduled commitment's config-version binding; concrete call hashing remains a Solidity assumption. |
| `check_ValidatorCannotPerformGuardianRecoveryAction` | Validator authority cannot mutate guardian/recovery state directly. | `validator_cannot_perform_guardian_recovery_action` | `setGuardianConfig`, recovery entry points | The Solidity property requires `OperationNotReady` and `InvalidModule`, then preserves guardian and validator identity. Lean abstracts guardian recovery mutation to an actor-gated transition; proof domains and recovery payloads remain concrete. |
| `check_GuardianCannotPerformValidatorAction` | Guardian authority cannot become spending authority. | `guardian_cannot_perform_validator_action` | `execute`, caller/EntryPoint guards | The Solidity property requires `OnlyEntryPoint` and preserves target, nonce, and account authority. Lean abstracts validator actions to an actor-gated transition; spending targets and proof domains remain concrete. |
| `check_PrivilegedAccountFunctionsRejectExternalCall` | Privileged lifecycle functions remain self-only. | `platform_actors_cannot_ordinary_execute_when_not_frozen` is related but insufficient. | schedule/cancel/install/uninstall/unfreeze self-call guards | The Solidity property requires each exact `OnlySelf` or `OperationNotReady` error and preserves schedules, nonce, validator identity, and account authority. The theorem covers ordinary execution, not privileged selector routing. |
| `check_MigrationDelayIsEnforced` | Migration cannot execute before its delay. | `migration_cannot_execute_before_delay` | `scheduleMigration`, `executeMigration`, `readyAt` | The Solidity property requires `OperationNotReady` and preserves all eight pending fields, migration nonce, configuration, and target state. Lean models delay and time advancement, but not exact revert data or block-time irregularity. |
| `check_MigrationHashBinding` | Migration executes only its committed call batch. | `migration_rejects_mismatched_calls_hash` | `pendingMigration.callsHash`, `executeMigration` | The Solidity property requires `InvalidMigration` and preserves the complete pending migration and nonce. Lean treats the hash as an abstract natural value; collision resistance and ABI encoding are concrete assumptions. |
| `check_MigrationBatchAtomicity` | A failed migration batch preserves pending state and effects. | `failed_batch_preserves_state` | `executeMigration`, batch loop, pending migration | The Solidity property requires the exact downstream target revert and preserves all pending fields, nonce, configuration, and target state. Lean abstracts migration effects to a committed effect counter and does not model EVM revert data. |
...
| `check_RecoveryDelayIsEnforced` | Recovery cannot execute before readiness. | `recovery_cannot_execute_before_delay` | `pendingRecoveries`, `executeRecovery`, `readyAt` | The Solidity property requires `RecoveryNotReady` and preserves all nine pending fields, recovery nonce, guardian configuration, and validator identity. Lean models delay and time advancement, but not exact revert data or block-time irregularity. |
...
| `check_FrozenAccountOnlyAllowsRecoveryCancel` | Frozen guardian cancellation removes pending recovery without spending. | `frozen_guardian_cancel_recovery_allowed` | `_isFrozenSafe`, `cancelRecovery` | The Solidity property executes through the EntryPoint, requires `AccountFrozen` for an ordinary schedule cancellation, and preserves the schedule and authority state after recovery cancellation. Guardian proof uniqueness and cancellation digest are abstracted. |
| `check_KeystoreUpdateRequiresController` | Only the identity controller can update keystore configuration. | `non_controller_keystore_update_preserves_state` | `LoomKeystore.updateConfig` | The Solidity property requires `Unauthorized` and preserves controller, all configuration fields, version, and config hash. Lean abstracts controller identity to a Boolean; exact revert data and storage layout remain concrete. |
| `check_SyncDelayIsEnforced` | Keystore sync cannot replace validators before delay. | `sync_cannot_execute_before_delay` | `proposeSync`, `executeSync`, `pendingSyncs.readyAt` | The Solidity property requires `SyncNotReady` and preserves all eleven pending fields, sync nonce, applied L1 version, guardian configuration, and validator identity. Lean abstracts sync payload and proof binding; exact revert data, block time, and keystore identity remain concrete. |
...
| `check_VaultWithdrawalDelayIsEnforced` | Protected assets cannot leave before vault delay. | `vault_withdrawal_before_delay_preserves_state` | vault policy hook, `scheduleVaultWithdrawal`, account execution | The Solidity property executes through the EntryPoint, requires `WithdrawalNotReady`, and preserves all pending fields, spending counters, and balances. Lean models abstract balances, pending state, and time; token semantics, hook classification, calldata binding, expiry, and config-version binding remain concrete. |
| `check_VaultGuardianCancellationGrantsNoSpendingAuthority` | Cancelling a withdrawal grants no spending authority. | `vault_guardian_cancellation_grants_no_spending_authority`, `approved_vault_guardian_cancellation_clears_pending` | `cancelVaultWithdrawalWithGuardians`, pending withdrawal | The Solidity property requires `WithdrawalNotPending` after cancellation and preserves spending counters, balances, validator identity, hook installation, and configuration. Lean abstracts guardian approval to a Boolean and spending authority to an identity; proof uniqueness, withdrawal identity, and token behavior remain concrete. **This property cannot cover cancellation-digest binding.** Its harness uses `FormalGuardianVerifier`, which accepts every signature, so the approval succeeds whatever the digest contains — including a replayed one. Digest binding, and specifically the per-slot instance nonce that stops a revealed approval from cancelling a re-scheduled withdrawal, is covered only by the concrete unit tests that use a real signing verifier: `testGuardianCancellationCannotBeReplayedAgainstARescheduledWithdrawal`, `testExecutedWithdrawalApprovalCannotCancelAnIdenticalLaterOne`, `testGuardianCancellationDoesNotCrossAccounts`, and the `vault-cancellation-instance-nonce` guard mutation. |
