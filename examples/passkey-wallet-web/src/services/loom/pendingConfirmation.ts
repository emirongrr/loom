...
import { validateUserOperationReceipt } from "./operationReceipt.ts";
...
export async function resumePendingConfirmation(input: {
  ...
  try {
    const transactionHash = validateUserOperationReceipt(body.result, input.operation.userOperationHash, input.account.account);
