...
export function App() {
  ...
  const createAccount = async (request: WalletCreationRequest) => {
    ...
    try {
      ...
      const initial = request.guardians ? await prepareInitialGuardianSetup({
        ...request.guardians,
        deployment,
        readVerifierCodeHash: verifier => readVerifierCodeHash(config, verifier)
      }) : undefined;
  ...
  if (!selected) return <><WalletLanding accounts={accounts} busy={busy} message={message} onCreate={createAccount} onImport={importAccount} onClearMessage={() => setMessage("")} onOpen={unlockAccount} onGuardianRecover={() => openRecovery()} /><button className="desktop-theme theme-toggle" onClick={() => setTheme(theme === "light" ? "dark" : "light")} aria-label={`Use ${theme === "light" ? "dark" : "light"} theme`}>{theme === "light" ? "◐" : "☀"}</button></>;
