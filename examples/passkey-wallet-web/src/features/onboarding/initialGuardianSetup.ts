...
/**
 * Finalize the initial guardian epoch immediately before passkey registration.
 * Verifier addresses must still match the deployment and their runtime code is
 * re-read so a stale form cannot bind a newly created account to changed code.
 */
export async function prepareInitialGuardianSetup(input: {
  ...
  for (const entry of input.entries) {
    ...
    if (!expected || expected.toLowerCase() !== entry.descriptor.verifier.toLowerCase()) {
      throw new Error(`This guardian does not use the deployment's ${entry.descriptor.kind.toUpperCase()} guardian verifier.`);
    }
    ...
    if (runtimeHash.toLowerCase() !== entry.descriptor.verifierCodeHash.toLowerCase()) {
      throw new Error("A guardian verifier's runtime code changed after it was selected.");
    }
  }
