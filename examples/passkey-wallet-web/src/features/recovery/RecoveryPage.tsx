...
import { assertPreparedRecoveryMatchesRequest, restorePreparedRecovery, verifyRecoveryResponseForProposal } from "./recoveryProposal";
...
export function RecoveryPage({ path, accounts, preferredGasPayerId, sourceWalletOpen = false, onClose, onNavigate, onRecovered }: {
  ...
  const publish = async () => {
    ...
    try {
      ...
      await publicClients.forEndpoint(config.rpcUrl).waitForTransactionReceipt({ hash });
  ...
  const publishWithLoomWallet = async () => {
    ...
    try {
      ...
      const result = await publishRecoveryValidatorWithLoomWallet({
        ...
        submit: submitInput => submitAccountCalls({ ...submitInput, pendingOperations })
      });
      await verifyPublication();
  ...
  return <main className="wallet-landing recovery-layout">
    ...
    <section className="saved-wallets" aria-labelledby="recovery-sessions-title">
      ...
      <details><summary>Resume from a portable request</summary><p className="form-note">Paste a versioned recovery request received through a file, QR, clipboard, or encrypted fragment. Unknown, altered, mismatched, and expired fields fail closed.</p><label className="field"><span>Recovery request</span><textarea rows={6} value={artifact} onChange={event => setArtifact(event.target.value)} placeholder='{"format":"loom.recovery-request",…}' /></label><button className="secondary" disabled={!artifact.trim()} onClick={() => void importRequest()}>Verify and save locally</button></details>
    </section>
  </main>;
}
...
function RecoveryProposalSessionView({ session, repository, onChanged, onRecovered, onBack }: {
  ...
  const rebuild = async (submitTransport?: Parameters<typeof createAccountGuardianClient>[0]["submitTransport"]) => {
    ...
    const client = createAccountGuardianClient({
      config, chainId: deployment.chainId, account: session.request.account, recoveryManager: session.request.recoveryManager,
    ...
    try {
      ...
      const submitTransport = {
        submit: async ({ to, data, value }: { to: `0x${string}`; data: `0x${string}`; value: bigint }) => {
          ...
          await publicClients.forEndpoint(config.rpcUrl).waitForTransactionReceipt({ hash: executionHash });
  ...
  const importResponse = async () => {
    ...
    try {
      ...
      const updated = transitionRecoverySession(session, { type: "response-added", response, approvalsRequired: session.request.guardianThreshold });
    ...
    try {
      ...
      const submitTransport = {
        submit: async ({ to, data, value }: { to: `0x${string}`; data: `0x${string}`; value: bigint }) => {
          ...
          await publicClients.forEndpoint(config.rpcUrl).waitForTransactionReceipt({ hash: transactionHash });
      ...
      const pending = await context.client.readPendingRecovery();
      if (!pending.pending || pending.readyAt <= 0n || pending.expiresAt <= pending.readyAt) throw new Error("The proposal transaction did not create the expected on-chain recovery.");
  ...
  const copyEncryptedLink = async () => {
    try {
      ...
      await navigator.clipboard.writeText(delivered.value); setMessage("Encrypted recovery link copied. Its decryption key stays in the URL fragment.");
    } catch { setMessage("Encrypted link could not be copied. Export the request file instead."); }
  };
