 ...
 * device that started the recovery. Finishing one no longer requires that
 * device: the validator is initialized when it is deployed (ADR-0025), so
 * execution carries no initializer and everything it needs is already on chain.
 *
 ...
 */
export function executionBlockers(input: { readonly lookup: RecoveryLookup }): readonly string[] {
  const { lookup } = input;
...
/**
 * Check the validator set against the hash the manager stored.
 *
 * `executeRecovery` compares it, so checking here means the button is only
 * offered when the call would actually be accepted. There is nothing else left
 * to check: the initializer is gone from this path entirely.
 */
