...
export function GuardianWorkspace({ account, inboundLink = "" }: { readonly account: AccountHandle; readonly inboundLink?: string }) {
  ...
  useEffect(() => {
    ...
    void createEncryptedLinkTransport<unknown>({ origin: window.location.origin }).receive(inboundLink)
      .then(payload => {
        if (payload && typeof payload === "object" && (payload as Record<string, unknown>).format === "loom.recovery-request") setRecoveryArtifact(inboundLink);
        else setLink(inboundLink);
      })
  ...
  const reviewRecovery = async () => {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Recovery request could not be reviewed."); }
  };
