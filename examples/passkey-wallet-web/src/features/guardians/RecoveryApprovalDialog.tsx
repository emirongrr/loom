...
export function RecoveryApprovalDialog({ request, capability, deployment, guardianAccount, onClose }: {
  ...
  const approve = async () => {
    ...
    try {
      ...
      const client = createAccountGuardianClient({ config, chainId: request.chainId, account: request.account, recoveryManager: request.recoveryManager, recoveryValidatorProvisioner: deployment.recoveryValidatorProvisioner, policyHook: deployment.policyHook });
  ...
  return <div className="dialog-backdrop" role="dialog" aria-modal="true" aria-label="Approve recovery"><div className="review-sheet">
