...
`index.html` uses bare module specifiers, so serve it through a bundler or dev
server that resolves `node_modules` (for example `vite`), fill `DEPLOYMENT` from
your deployment manifest (`loom deploy inspect --manifest …`), and open it over
HTTPS or `localhost` — WebAuthn requires a secure context and a real platform
authenticator.
