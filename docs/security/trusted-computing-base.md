...
| Installed migration module | Its successful consumption decision authorizes the exact caller-supplied batch executed by the account's narrow migration entry point. A defect can widen migration authorization, although the module cannot call the account or transfer assets independently. | At most one; installed or removed through the delayed module-management path. Deployment profiles must pin reviewed immutable runtime code. |
