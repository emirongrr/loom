...
| EIP-7702 delegation | Lets an EOA preserve its address while using Loom runtime code | Malicious persistent delegation, wrong template, third-party initialization of an uninitialized delegated account, dirty storage from a prior delegation target, cross-chain authorization blast radius | Self-only one-time delegated initialization, proxy bootstrap initializer restricted to the proxy-construction context, template bytecode verification, chain-specific authorization, explicit client warnings. Dirty prior-delegation storage remains a client-side check; see `docs/decisions/0015-initialization-context-separation.md` |
