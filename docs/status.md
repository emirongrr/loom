...
| ERC-1271 routing | `src/LoomAccount.sol`, validators, `test/regression/SecurityRegression.t.sol` | Boundary implemented | The account routes `isValidSignature` to the named validator, but **every bundled validator returns false**: the primary ones cannot classify a bare hash against a policy, and the session ones authorize UserOperations only. No in-repository configuration produces a valid ERC-1271 signature, and `supportsInterface` correctly reports `false` for `0x1626ba7e`. Off-chain message signing (SIWE, Permit2, marketplace orders, governance) requires a non-bundled validator through the ERC-7579 shim. |
...
| Migration | `src/MigrationModule.sol`, `test/integration/Migration.t.sol`, `docs/design/lifecycle.md` | Implemented | A passive account-scoped ledger stores delayed, hash-committed exit batches; it has no callback into account execution. The account consumes its record and reuses the common hook-mediated execution engine. This is one-chain migration, not cross-chain sync or a terminal state. The committed destination is reviewable metadata, not a requirement that a batch call target it. |
| EIP-7702 delegated account initialization | `src/LoomAccount.sol`, `test/integration/EIP7702Integration.t.sol`, `docs/design/eip-7702.md`, `docs/decisions/0015-initialization-context-separation.md` | Implemented | Requires self-only initialization by the delegated EOA; the proxy bootstrap initializer is restricted to the proxy-construction context and cannot reach a delegated account. Storage left by a prior delegation target remains a client-side check. |
...
| OP Stack L2 keystore verifier | `src/keystore/OPStackL2KeystoreVerifier.sol`, `test/unit/OPStackL2KeystoreVerifier.t.sol`, `test/fork/OPStackL1BlockPredeploy.fork.t.sol` | Partially implemented | Roots proofs in `L1Block.hash()` via a caller-supplied block header; the predeploy publishes no state root. An earlier version assumed it did and was inert on every real OP Stack chain. Predeploy conformance is asserted only when `OP_STACK_RPC_URL` is set, the committed proof fixture is locally generated rather than real `eth_getProof` output, and production deployment still requires audit, profile evidence, and live rehearsal. |
