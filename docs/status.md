...
| Recovery validator provisioning | `src/validators/P256RecoveryValidatorFactory.sol`, `test/unit/P256RecoveryValidatorFactory.t.sol` | Implemented | Permissionless deterministic P-256 validator instances; each deployment must publish the factory and code-hash profile. |
| Guardian verifier types | `src/recovery/ECDSAGuardianVerifier.sol`, `src/recovery/P256GuardianVerifier.sol`, `src/recovery/ERC1271GuardianVerifier.sol`, `test/unit/GuardianVerifier.t.sol` | Implemented | The wallet resolves one entered address during setup and pins an explicit verifier; production guardian setup still depends on audited deployment evidence and client ceremony. |
