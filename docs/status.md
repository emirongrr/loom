...
| Sovereign migration | `src/LoomAccount.sol`, `test/integration/SovereignMigration.t.sol`, `docs/design/lifecycle.md` | Implemented | A delayed, hash-committed exit batch on one chain, not cross-chain sync and not a terminal state: the source account stays operational afterwards. The committed destination is a reviewable declaration, not an execution constraint — no call in the batch is required to target it. |
