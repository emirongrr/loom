...
> Store a random, stable `accountHandle` for the account in the discoverable
...
```text
During recovery
account A ──► accountHandle H ──► new passkey credential source
...
- `accountHandle` is a random, non-zero 32-byte value generated by a CSPRNG.
...
- Use `accountHandle` consistently in the new generation.
...
The selected generation now uses `accountHandle`, the 62-byte v3 envelope,
factory-scoped forward and reverse registry bindings, post-registration
recovery assertions, and a typed discovery result. BE/BS is retained as
Security-screen availability guidance rather than authority or a guardian
recovery gate. Sponsored deployments use private activation submission;
self-funded counterfactual activation remains an explicit alternative. The
remaining release requirement is the real clean-device lifecycle rehearsal on
the canonical deployment and its recorded evidence.
