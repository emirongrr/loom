...
| Signer agility | Validators are modular; guardian leaves bind salted key commitments to a verifier address and its `codehash` (which does not by itself prove immutability; see `docs/design/guardians.md`) | Modular accounts can install new signer modules | Independently audit additional guardian verifiers and migration ceremonies |
