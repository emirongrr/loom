...
| MEDIUM-02 Hook denial of service | Mitigated with a narrow bypass that can only schedule and execute the 72-hour delayed removal of an installed hook. Every other normal and scheduled execution uses the pre-check hook snapshot. Hook availability during the delay remains a residual risk. |
